...
### ✏️ 하이라이트 기능
- `add_highlight` - 새로운 하이라이트 추가 (색상과 스타일 선택 가능, 기본값: 노란색 하이라이트)
...
(색상과 스타일 미지정 시 자동으로 노란색 하이라이트 적용)
```
...
```typescript
{
  ...
  color: string;             // 색상 (기본값: "#FFEB3B" 노란색)
  style: 'highlight' | 'underline';  // 스타일 (기본값: "highlight")
  startOffset: number;       // 텍스트 시작 위치
  endOffset: number;         // 텍스트 끝 위치
  contextBefore: string;     // 앞 문맥
  contextAfter: string;      // 뒷 문맥
  firstWords: string;        // 처음 3단어 (매칭용)
  lastWords: string;         // 마지막 3단어 (매칭용)
